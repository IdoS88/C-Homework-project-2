...
int main()
{
	...
	if( compress == 1)
		initManagerAndAirline(&manager, &company, AIRLINE_COMPRESS_FILE_NAME ,compress );
	else	
		initManagerAndAirline(&manager, &company, AIRLINE_FILE_NAME , compress);
	...
	if (compress == 1)
		writeAirLineToBinaryFileCompress(AIRLINE_COMPRESS_FILE_NAME, &company);
	else
...
int initManagerAndAirline(AirportManager* pManager, Airline* pCompany,const char* ailineFileName , int isCompress)
{
	...
	if (res == FROM_FILE)
		return initAirlineFromFile(pCompany, pManager, ailineFileName , isCompress);
	else
