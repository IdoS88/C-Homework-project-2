...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName , int isCompress)
{
	...
	if (isCompress)
	{
		if (readAirLineFromBinaryFileCompress( fileName , pComp))
			return initDateList(pComp);
	}

	else if (loadAirlineFromFile(pComp, pManaer, fileName))
		return initDateList(pComp);
